    ...
    if cmd in ['r','roll']:
        ...
        if 'adv' in args[0]['value']:
            ...
            for arg in roll_args:
                if 'd' in arg:
                    roll_args[c] = '2d'+roll_args[c].split('d')[1][:len(roll_args[c].split('d')[1])-1]+'kh1'+roll_args[c].split('d')[1][len(roll_args[c].split('d')[1])-1]
                c+=1
        elif 'dis' in args[0]['value']:
            c = 0
            for arg in roll_args:
                if 'd' in arg:
                    roll_args[c] = '2d'+roll_args[c].split('d')[1][:len(roll_args[c].split('d')[1])-1]+'kl1'+roll_args[c].split('d')[1][len(roll_args[c].split('d')[1])-1]
                c+=1
