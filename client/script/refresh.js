...
async function refresh(data) {
    ...
    if ((endpoints.campaigns || start) && window.location.pathname.includes('campaigns')) {
        ...
        if ($('#campaign-panel').hasClass('active')) {
            var ind = null;
            if ($('#campaign-panel').prop('data-editing')) {
                for (var c = 0; c < oc.length; c++) {
                ...
                if (ind != null) { loadCampaign(oc[ind], $('#campaign-panel').prop('data-editing')); }
            } else {
                for (var c = 0; c < pc.length; c++) {
                    if (pc[c].id == $('#campaign-panel').attr('data-id')) {
                        ind = c;
                        break;
                    }
                }
                if (ind != null) { loadCampaign(pc[ind], $('#campaign-panel').prop('data-editing')); }
            }
